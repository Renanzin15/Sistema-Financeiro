...
def gerar_licencas(user_id):
    ...
    licencas = con.execute(
        "SELECT id, nome, valor_centavos, periodicidade, proximo_vencimento, tipo, conta_fatura_id "
        "FROM licencas WHERE user_id=?",
        (user_id,)
    ).fetchall()
    for lid, nome, valor, periodicidade, prox, tipo, fatura_id in licencas:
        if not prox:
            continue
        guard = 0
...
def criar_licenca(item: NovaLicenca, user_id: str = Depends(exigir_login)):
    ...
    if item.conta_fatura_id is not None:
        ...
        if f is None or (f[0] or "simples") != "fatura":
            ...
            raise HTTPException(status_code=400, detail="Escolha uma fatura de cartão válida.")
    con.execute(
        "INSERT INTO licencas (nome, fornecedor, valor_centavos, periodicidade, proximo_vencimento, tipo, conta_fatura_id, user_id) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (item.nome.strip(), (item.fornecedor or "").strip() or None, item.valor_centavos,
         item.periodicidade, item.proximo_vencimento, item.tipo, item.conta_fatura_id, user_id)
    )
    con.commit()
    con.close()
