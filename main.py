...
def aplicar_regras_salario(con, descricao, valor_centavos, data):
    ...
    if not alvo:
        return
    regras = con.execute(
        "SELECT modo, valor, caixinha_id FROM regras_salario WHERE lower(gatilho)=?",
        (alvo,)
    ).fetchall()
    for modo, valor, cxid in regras:
        if cxid is None:
            continue
        existe = con.execute("SELECT id FROM caixinhas WHERE id=?", (cxid,)).fetchone()
        if existe is None:
            continue
        if modo == "percentual":
        ...
        if quantia <= 0:
            continue
        con.execute(
            "INSERT INTO lancamentos (tipo, valor_centavos, descricao, caixinha_id, data) VALUES ('alocacao', ?, ?, ?, ?)",
...
def gerar_recorrentes_do_mes():
    ...
    recorrentes = con.execute(
        "SELECT nome, valor_centavos, dia_vencimento, tipo, conta_fatura_id FROM recorrentes"
    ).fetchall()
    for nome, valor, dia, tipo, fatura_id in recorrentes:
        vencimento = f"{ano:04d}-{mes:02d}-{dia:02d}"
        if fatura_id is not None:
            fatura = con.execute(
                "SELECT id, tipo_conta, paga FROM contas WHERE id=?", (fatura_id,)
            ).fetchone()
            ...
            if fatura is not None and (fatura[1] or "simples") == "fatura":
                if fatura[2] == 0:  # só mexe na fatura se ainda não foi paga
                    ja_item = con.execute(
                        "SELECT id FROM fatura_itens WHERE conta_id=? AND descricao=? AND data LIKE ?",
                        (fatura_id, nome, mes_str + "%")
                    ).fetchone()
                    if ja_item is None:
                        con.execute(
...
def listar_caixinhas(_=Depends(exigir_login)):
    con = conectar()
    caixas = con.execute("SELECT id, nome, banco_id, meta_centavos, meta_prazo FROM caixinhas").fetchall()
    resultado = []
    for c in caixas:
        cid = c[0]
        meta = c[3] or 0
...
def definir_meta(caixinha_id: int, item: DefinirMeta, _=Depends(exigir_login)):
    ...
    con.execute("UPDATE caixinhas SET meta_centavos=?, meta_prazo=? WHERE id=?", (meta, prazo, caixinha_id))
    con.commit()
    con.close()
...
def listar_contas(_=Depends(exigir_login)):
    ...
    linhas = con.execute(
        "SELECT id, nome, valor_centavos, vencimento, tipo, paga, caixinha_paga_id, tipo_conta FROM contas WHERE arquivada=0 OR arquivada IS NULL"
    ).fetchall()
    resultado = []
    for l in linhas:
...
def listar_contas_arquivadas(_=Depends(exigir_login)):
    con = conectar()
    linhas = con.execute(
        "SELECT id, nome, valor_centavos, vencimento, tipo, paga, caixinha_paga_id, tipo_conta FROM contas WHERE arquivada=1 ORDER BY vencimento DESC"
    ).fetchall()
    resultado = []
    for l in linhas:
        tipo_conta = l[7] or "simples"
...
def arquivar_conta(conta_id: int, _=Depends(exigir_login)):
    con = conectar()
    conta = con.execute("SELECT paga FROM contas WHERE id=?", (conta_id,)).fetchone()
    if conta is None:
        con.close()
    ...
    con.execute("UPDATE contas SET arquivada=1 WHERE id=?", (conta_id,))
    con.commit()
    con.close()
    return {"status": "conta arquivada"}

@app.post("/contas/{conta_id}/desarquivar")
def desarquivar_conta(conta_id: int, _=Depends(exigir_login)):
    con = conectar()
    existe = con.execute("SELECT id FROM contas WHERE id=?", (conta_id,)).fetchone()
    if existe is None:
        con.close()
        raise HTTPException(status_code=404, detail="Essa conta não existe.")
    con.execute("UPDATE contas SET arquivada=0 WHERE id=?", (conta_id,))
    con.commit()
    con.close()
    return {"status": "conta desarquivada"}

@app.post("/contas/arquivar-antigas")
def arquivar_antigas(_=Depends(exigir_login)):
    ...
    cur = con.execute(
        "UPDATE contas SET arquivada=1 WHERE paga=1 AND (arquivada=0 OR arquivada IS NULL) AND vencimento < ?",
        (inicio_mes,)
    )
    qtd = cur.rowcount
    con.commit()
...
def criar_recorrente(item: NovaRecorrente, _=Depends(exigir_login)):
    ...
    if item.conta_fatura_id is not None:
        f = con.execute("SELECT tipo_conta FROM contas WHERE id=?", (item.conta_fatura_id,)).fetchone()
        if f is None or (f[0] or "simples") != "fatura":
            con.close()
            raise HTTPException(status_code=400, detail="Escolha uma fatura de cartão válida.")
    con.execute(
        "INSERT INTO recorrentes (nome, valor_centavos, dia_vencimento, tipo, conta_fatura_id) VALUES (?, ?, ?, ?, ?)",
        (item.nome, item.valor_centavos, item.dia_vencimento, item.tipo, item.conta_fatura_id)
    )
    con.commit()
    con.close()
...
def listar_recorrentes(_=Depends(exigir_login)):
    con = conectar()
    linhas = con.execute(
        "SELECT id, nome, valor_centavos, dia_vencimento, tipo, conta_fatura_id FROM recorrentes"
    ).fetchall()
    con.close()
    return [
...
def vincular_fatura(recorrente_id: int, item: VincularFatura, _=Depends(exigir_login)):
    con = conectar()
    rec = con.execute("SELECT id, nome FROM recorrentes WHERE id=?", (recorrente_id,)).fetchone()
    if rec is None:
        con.close()
    ...
    if item.conta_fatura_id is not None:
        ...
        con.execute(
            "DELETE FROM contas WHERE nome=? AND paga=0 AND (tipo_conta='simples' OR tipo_conta IS NULL) AND vencimento LIKE ?",
            (rec[1], mes_str + "%")
        )
    con.execute("UPDATE recorrentes SET conta_fatura_id=? WHERE id=?", (item.conta_fatura_id, recorrente_id))
    con.commit()
    con.close()
...
@app.get("/regras")
def listar_regras(_=Depends(exigir_login)):
    con = conectar()
    linhas = con.execute(
        "SELECT id, gatilho, modo, valor, caixinha_id FROM regras_salario ORDER BY id"
    ).fetchall()
    con.close()
    return [
...
@app.post("/regras")
def criar_regra(item: NovaRegra, _=Depends(exigir_login)):
    ...
    con = conectar()
    existe = con.execute("SELECT id FROM caixinhas WHERE id=?", (item.caixinha_id,)).fetchone()
    if existe is None:
        con.close()
        raise HTTPException(status_code=400, detail="Essa caixinha não existe.")
    con.execute(
        "INSERT INTO regras_salario (gatilho, modo, valor, caixinha_id) VALUES (?, ?, ?, ?)",
        (gatilho, item.modo, item.valor, item.caixinha_id)
    )
    con.commit()
    con.close()
...
def apagar_regra(regra_id: int, _=Depends(exigir_login)):
    con = conectar()
    existe = con.execute("SELECT id FROM regras_salario WHERE id=?", (regra_id,)).fetchone()
    if existe is None:
        con.close()
        raise HTTPException(status_code=404, detail="Essa regra não existe.")
    con.execute("DELETE FROM regras_salario WHERE id=?", (regra_id,))
    con.commit()
    con.close()
...
def wrapped_status(_=Depends(exigir_login)):
    con = conectar()
    r = con.execute("SELECT valor FROM config WHERE chave='wrapped_visto_ate'").fetchone()
    con.close()
    return {"visto_ate": r[0] if r else None}

@app.post("/wrapped-visto")
def wrapped_visto(item: WrappedVisto, _=Depends(exigir_login)):
    con = conectar()
    con.execute("INSERT OR REPLACE INTO config (chave, valor) VALUES ('wrapped_visto_ate', ?)", (item.mes,))
    con.commit()
    con.close()
    return {"status": "ok"}
...
def importar_analisar(item: ImportarAnalise, _=Depends(exigir_login)):
    con = conectar()
    caixinhas = [(c[0], (c[1] or "").lower()) for c in con.execute("SELECT id, nome FROM caixinhas").fetchall()]
    con.close()
...
def importar_confirmar(item: ImportarConfirmar, _=Depends(exigir_login)):
    ...
    for l in item.linhas:
        ...
        if l.tipo == "entrada":
            ...
            con.execute(
                "INSERT INTO lancamentos (tipo, valor_centavos, descricao, caixinha_id, data) VALUES ('entrada', ?, ?, NULL, ?)",
                (l.valor_centavos, l.descricao, data)
            )
        elif l.caixinha_id:
            con.execute(
                "INSERT INTO lancamentos (tipo, valor_centavos, descricao, caixinha_id, data) VALUES ('pagamento', ?, ?, ?, ?)",
                (l.valor_centavos, l.descricao, l.caixinha_id, data)
            )
        else:
            con.execute(
                "INSERT INTO lancamentos (tipo, valor_centavos, descricao, caixinha_id, data) VALUES ('saida_livre', ?, ?, NULL, ?)",
                (l.valor_centavos, l.descricao, data)
            )
        n += 1
    con.commit()
